...
- **关键词正则匹配**：广告拦截、群组监听、Web/RSS 监控的关键词和屏蔽词统一支持两种写法：普通词直接写（大小写不敏感的包含匹配），`/正则/` 按正则匹配（例如 `/VPS|优惠|限免/`）。正则写错会自动退回普通匹配，老配置不受影响。
- **Web/RSS 精准匹配**：关键词和屏蔽词只匹配标题、正文；作者、发布时间、分类不再混入。RSS 正文只保留摘要与正文，Web 正文会去除开头重复的标题。
- **私聊广告拦截留痕**：关闭「命中后自动拉黑」时，命中关键词的消息也会在收件箱标记 `spam: ...`，方便排查；开启自动拉黑时行为不变。
- **面板 UI**：监控表单、群监听表单和拦截规则页的关键词输入框标注正则写法并加提示；群监听「监听来源」说明改为提示文字，AI 超时/推送间隔/去重窗口合并为一行。
- 测试桩补齐 `aiogram.types` 新增导入和 `router.callback_query`；本轮变更未执行本地测试。

### 2026-09-21 更新

- **图片验证码人机验证**：新用户首次私聊 Bot 需先通过 4 位图片验证码（PIL 本地生成，无新增依赖），通过后消息才会转交管理员，支持「换一张」按钮和 10 分钟过期（可配置）。
- **防爆破设计**：答错一次当前验证码立即作废并自动发送新图，验证阶段同样受限流约束，脚本高频刷码直接被限流挡住。
- **私聊自动回复**：管理员忙碌/离线时可自动答复私聊用户（文案自定义），按用户去重不会刷屏，Web 面板可视化配置。
- **静默拉黑**：广告关键词命中的用户拉黑后完全静默——不回复、不通知管理员，仅面板留痕，拉黑用户无任何感知。
- 验证管理：Web 面板 `/rules` 三卡片管理（广告拦截/人机验证/自动回复），用户管理页可人工验证/撤销验证，管理员命令 `/verify` `/unverify` `/verifylist`。
- 离线端到端测试 19 项覆盖完整验证流程（验证码门、防爆破、自动回复、静默拉黑、功能开关）。
- README 同步更新。
...
- 保留「频道媒体下载/转发」功能。
- Web 面板导航保留「频道媒体」入口，修复导航标签被遮住的问题。
...
- 关键词支持普通词或 `/正则/`（忽略大小写），例如 `/博彩|彩票|开户/`。
...
- 关键词和屏蔽词支持普通词或 `/正则/`（忽略大小写），例如 `/VPS|优惠/`。
- 关键词和屏蔽词只匹配标题、正文；作者、发布时间、分类不参与。
- 单个监控可关闭 Telegram 推送，只记录到 Web 推送历史。
