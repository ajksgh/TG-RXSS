...
def install_import_stubs() -> None:
    ...
    modules["aiogram"].F = SimpleNamespace(data=None)
    ...
    modules["aiogram.types"].BufferedInputFile = object
    modules["aiogram.types"].CallbackQuery = object
    modules["aiogram.types"].InlineKeyboardButton = object
    modules["aiogram.types"].InlineKeyboardMarkup = object
...
class MonitorMessageCleanupTest(unittest.TestCase):
    ...
    def test_spam_hit_marks_inbox_without_auto_block(self) -> None:
        old_bot = app.bot
        old_admin_chat_id = app.admin_chat_id
        old_admin_chat_ids = app.admin_chat_ids
        old_config = app.config
        ...
        app.config = {
            "bot": {
                ...
                "spam_filter": {"enabled": True, "auto_block": False, "keywords": ["博彩"]},
                "verification": {"enabled": False},
                "auto_reply": {"enabled": False, "text": ""},
            }
        }
        uid = 777001
        app.upsert_user(uid, "Spammer", "spammer")
        app.set_block(uid, False)
        app.set_verified(uid, True)
        answers: list[str] = []

        async def answer(text: str) -> None:
            answers.append(text)

        async def copy_to(chat_id: int, reply_to_message_id: int | None = None):
            return SimpleNamespace(message_id=3004)

        msg = SimpleNamespace(
            chat=SimpleNamespace(id=uid, type="private"),
            from_user=SimpleNamespace(id=uid, first_name="S", last_name=None, username="spammer"),
            text="最高博彩平台",
            caption=None,
            message_id=66,
            content_type="text",
        ...
        try:
            asyncio.run(app.user_message(msg))
            self.assertFalse(app.is_blocked(uid))
            self.assertEqual([999], fake_bot.sent_chat_ids)
            row = app.get_inbox_message(1)
            self.assertIsNotNone(row)
            self.assertEqual(1, row["forwarded"])
            self.assertTrue((row["error"] or "").startswith("spam-hit:"))
            self.assertIn("已转交管理员。", answers)
        finally:
            app.bot = old_bot
            app.admin_chat_id = old_admin_chat_id
            app.admin_chat_ids = old_admin_chat_ids
...
class PanelHtmlContractTest(unittest.TestCase):
    ...
    def test_monitor_form_documents_regex_syntax(self) -> None:
        html = app.monitor_form_html()
        self.assertIn("支持 /正则/", html)
        self.assertIn('class=field-hint', html)

    def test_group_monitor_form_documents_regex_syntax(self) -> None:
...
class KeywordRegexTest(unittest.TestCase):
    def test_keyword_hits_plain_and_regex(self) -> None:
        keywords = ["VPS", "/优惠|特价/", "/^限免/"]
        self.assertEqual(["VPS", "/优惠|特价/"], app.keyword_hits("今晚 VPS 有优惠", keywords))
        self.assertEqual(["/优惠|特价/"], app.keyword_hits("今晚有特价", keywords))
        self.assertEqual(["/^限免/"], app.keyword_hits("限免活动", keywords))
        self.assertEqual([], app.keyword_hits("没有命中", keywords))

    def test_keyword_hits_invalid_regex_falls_back_to_plain(self) -> None:
        self.assertEqual(["/vps(/"], app.keyword_hits("这里有 vps( 优惠", ["/vps(/"]))
        self.assertEqual([], app.keyword_hits("普通文本", ["/vps(/"]))

    def test_spam_keyword_hits_supports_regex(self) -> None:
        old_config = app.config
        app.config = {"bot": {"spam_filter": {"enabled": True, "keywords": ["/博彩|彩票/"]}}}
        try:
            self.assertEqual(["/博彩|彩票/"], app.spam_keyword_hits("正规博彩平台"))
            self.assertEqual([], app.spam_keyword_hits("今天天气不错"))
        finally:
            app.config = old_config

    def test_item_blocked_supports_regex_rules(self) -> None:
        item = app.MonitorItem(key="k", title="VPS 优惠", link="https://x", text="VPS 优惠")
        blocked, reason = app.item_blocked(item, {"keywords": [], "exclude_keywords": ["/求带|带带/"]})
        self.assertFalse(blocked)
        item = app.MonitorItem(key="k2", title="求带上车", link="https://x", text="求带上车")
        blocked, reason = app.item_blocked(item, {"keywords": [], "exclude_keywords": ["/求带|带带/"]})
        self.assertTrue(blocked)
        self.assertIn("/求带|带带/", reason)
        item = app.MonitorItem(key="k3", title="今晚有特价", link="https://x", text="今晚有特价")
        monitor = {"keywords": ["/优惠|特价/"], "exclude_keywords": []}
        self.assertEqual((False, ""), app.item_blocked(item, monitor))
...
class MonitorRuntimeAndUpdateTest(unittest.TestCase):
    ...
    def test_handle_group_keyword_message_supports_regex(self) -> None:
        old_config = app.config
        old_bot = app.bot
        old_admin_chat_ids = app.admin_chat_ids
        fake_bot = FakeBot()
