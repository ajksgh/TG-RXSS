...
def get_or_create_channel_media_client(client_type: str = "channel_media", proxy: str = "") -> Any:
    if TelegramClient is None or StringSession is None:
        return None
    cache_key = f"{client_type}|{proxy.strip()}"
    if cache_key in channel_media_clients:
        c = channel_media_clients[cache_key]
        if c.is_connected():
            return c
    api_id, api_hash, session = user_session_config()
...
async def disconnect_channel_media_client(client_type: str | None = None) -> None:
    if client_type is None:
        clients = list(channel_media_clients.values())
        channel_media_clients.clear()
    else:
            ...
            async with semaphore:
                ...
                for attr in (getattr(getattr(msg.media, "document", None), "attributes", None) or []):
                    if hasattr(attr, "file_name") and attr.file_name:
                        ...
                        break
                if file_name:
                    file_name = ospath.basename(file_name.replace("\\", "/")).strip()
                    if file_name in {".", ".."}:
                        file_name = ""
                if not file_name:
...
async def send_text_to_user(user_id: int, text: str, source: str = "web") -> int:
    ...
    sent = await bot.send_message(user_id, text.strip(), parse_mode=None)
...
async def user_message(message: Message) -> None:
    ...
    try:
        ...
        if spam_hits:
            # 未开启自动拉黑时只标记命中，转发照常；放在转发成功后，避免被清空
            mark_inbox_error(inbox_id, "spam-hit: " + ", ".join(spam_hits))
            logger.info("spam keyword hit user_id=%s inbox_id=%s hits=%s", uid, inbox_id, spam_hits)
...
def keyword_hits(text: str, keywords: list[str]) -> list[str]:
    """普通关键词做小写子串匹配；/pattern/ 做正则匹配（忽略大小写）。"""
    hits: list[str] = []
    low = text.lower()
    for k in keywords:
        if not k:
...
def item_keyword_hits(item: MonitorItem, keywords: list[str]) -> list[str]:
    """标题和正文分别匹配；作者、发布时间、分类不参与关键词命中。"""
    hits: list[str] = []
    for field in (item.title, item.text):
        for hit in keyword_hits(field, keywords):
            if hit not in hits:
                hits.append(hit)
    return hits
...
def item_blocked(item: MonitorItem, monitor: dict[str, Any]) -> tuple[bool, str]:
    exclude_hits = item_keyword_hits(item, monitor.get("exclude_keywords") or [])
    if exclude_hits:
...
def parse_web_items(monitor: dict[str, Any], body: str) -> list[MonitorItem]:
    ...
    for node in nodes:
        ...
        if title and text.startswith(title):
            text = text.removeprefix(title).strip()
        price = None
        stock = None
...
def parse_rss_items(monitor: dict[str, Any], body: str) -> list[MonitorItem]:
    ...
    for e in feed.entries[:100]:
        ...
        body = " ".join(part for part in (summary, content) if part).strip()
        items.append(MonitorItem(key=key, title=title, link=link, text=body, author=author, published=published, category=category))
    return items
    ...
    try:
        ...
        for item in items:
            ...
            if blocked:
                ...
                continue
            hits = item_keyword_hits(item, keywords)
...
def panel_cookie_secure(request: Request) -> bool:
    forced = os.getenv("WEB_PANEL_COOKIE_SECURE", "").strip().lower()
...
(function(){
  ...
  document.addEventListener('mousemove', function(event) {
    ...
    var target = event.target.closest('.card,.step,nav section');
    if (!target) return;
...
p{{margin:0 0 22px;color:var(--muted);line-height:1.5;font-weight:500}}
label{{display:block;margin:14px 0 7px;color:var(--ink);font-size:12px;font-weight:600;text-transform:none;letter-spacing:0}}
input{{width:100%;border:1px solid var(--line);border-radius:8px;background:rgba(255,255,255,.9);color:var(--ink);padding:11px 12px;font-size:14px;font-weight:500;outline:none;transition:border-color .16s var(--ease),box-shadow .16s var(--ease)}}
input:focus{{border-color:var(--blue);box-shadow:0 0 0 3px rgba(59,91,219,.14)}}
button{{width:100%;margin-top:22px;border:1px solid transparent;border-radius:8px;padding:11px 16px;background:var(--blue);color:#fff;font-weight:700;font-size:14px;text-transform:none;letter-spacing:0;cursor:pointer;box-shadow:0 6px 20px rgba(59,91,219,.25);transition:background-color .16s var(--ease)}}
button:hover{{background:#3352c7}}
...
html[data-theme="dark"] button{{background:var(--blue);box-shadow:0 6px 20px rgba(94,106,210,.3)}}
html[data-theme="dark"] button:hover{{background:#6872D9}}
...
def monitor_from_form(
    ...
    m: dict[str, Any] = {
        ...
        "interval_seconds": max(
            int(DEFAULT_MONITOR_INTERVAL_SECONDS if interval_seconds is None else interval_seconds),
...
nav section:nth-child(3)>b{{background:rgba(59,91,219,.12);color:var(--blue)}}
nav section:nth-child(4)>b{{background:rgba(232,180,32,.16);color:#8a6d00}}
nav a:hover{{text-decoration:none;background:rgba(20,22,28,.05)}}
.logout{{color:var(--red)!important}}
.top{{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:20px;padding-bottom:14px;border-bottom:1px solid var(--line)}}
.top h1{{margin:0;font-size:26px;line-height:1.1;color:var(--ink);font-weight:800;text-transform:none;letter-spacing:0}}
...
h2,h3{{font-weight:800;text-transform:none;letter-spacing:0}}
h2{{font-size:22px}}
h3{{font-size:15px;border-bottom:1px solid var(--line);padding-bottom:6px;margin-top:20px}}
input,select,textarea{{width:100%;box-sizing:border-box;background:rgba(255,255,255,.9);color:var(--ink);border:1px solid var(--line);border-radius:8px;padding:10px 11px;outline:none;font-size:14px;font-weight:500;transition:border-color .14s var(--ease),box-shadow .14s var(--ease)}}
input:focus,select:focus,textarea:focus{{border-color:var(--blue);box-shadow:0 0 0 3px rgba(59,91,219,.14)}}
textarea{{min-height:116px;font-family:ui-monospace,"Cascadia Mono",Consolas,monospace}}
label{{display:block;margin:10px 0 5px;color:var(--ink);font-weight:600;font-size:12px;text-transform:none;letter-spacing:0}}
.check-row{{display:flex;gap:10px;align-items:center;flex-wrap:wrap}}
.check-row label{{display:flex;gap:7px;align-items:center;margin:0;padding:8px 10px;border:1px solid var(--line);border-radius:8px;background:rgba(255,255,255,.6);transition:background-color .14s var(--ease)}}
.check-row label:hover{{background:rgba(59,91,219,.06)}}
...
.bot-links{{margin-top:18px;padding-top:12px;border-top:1px solid var(--line);display:flex;gap:8px;align-items:center;flex-wrap:wrap}}
...
html[data-theme="dark"] .btn.danger{{background:var(--red)}}
html[data-theme="dark"] .btn.ok{{background:rgba(215,197,107,.9);color:#1a1400}}
html[data-theme="dark"] input,html[data-theme="dark"] select,html[data-theme="dark"] textarea{{background:rgba(15,15,18,.7)}}
html[data-theme="dark"] input:focus,html[data-theme="dark"] select:focus,html[data-theme="dark"] textarea:focus{{box-shadow:0 0 0 3px rgba(94,106,210,.22);border-color:rgba(94,106,210,.55)}}
html[data-theme="dark"] table,html[data-theme="dark"] tr:nth-child(even) td{{background:transparent}}
html[data-theme="dark"] th{{background:rgba(94,106,210,.16);color:var(--ink)}}
html[data-theme="dark"] .badge{{background:rgba(94,106,210,.2)}}
html[data-theme="dark"] .badge.warn{{background:rgba(215,197,107,.16);color:var(--yellow)}}
html[data-theme="dark"] .msg{{background:rgba(215,197,107,.1);border-color:rgba(215,197,107,.3);color:var(--ink)}}
html[data-theme="dark"] .step-no{{background:rgba(94,106,210,.2);color:var(--blue)}}
html[data-theme="dark"] .check-row label{{background:rgba(255,255,255,.05)}}
html[data-theme="dark"] .check-row label:hover{{background:rgba(94,106,210,.12)}}
html[data-theme="dark"] pre{{background:#0a0a0d;border:1px solid var(--line)}}
html[data-theme="dark"] .brand,html[data-theme="dark"] .top,html[data-theme="dark"] h3,html[data-theme="dark"] .bot-links{{border-color:var(--line)}}
...
def monitor_form_html(m: dict[str, Any] | None = None, idx: int | None = None) -> str:
    m = m or {"type": "web", "interval_seconds": DEFAULT_MONITOR_INTERVAL_SECONDS, "notify_on": {"keyword_match": True, "new_item": True, "price_change": True, "stock_change": True}, "selectors": {}}
...
<label>标题/正文关键词（一行一个，支持 /正则/）</label><textarea name=keywords placeholder='普通词写 VPS；正则写 /VPS|优惠|限免/'>{html_escape(keywords)}</textarea>
<small class=field-hint>只匹配标题和正文；作者、发布时间、分类不参与。普通词按大小写不敏感的包含匹配；/…/ 按正则匹配，例如 /VPS|优惠/、/^限免/。</small>
<label>排除词（一行一个，支持 /正则/）</label><textarea name=exclude_keywords placeholder='普通词写 求带；正则写 /(求带|带带)/' style='min-height:72px'>{html_escape(exclude_keywords)}</textarea>
<small class=field-hint>命中排除词时直接跳过这条内容，不推送。规则写法与关键词一致。</small>
...
<div class=grid><div><label>监听来源</label><select name=listen_source><option value=bot {'selected' if str(m.get('listen_source', 'bot')) == 'bot' else ''}>Bot</option><option value=user_session {'selected' if str(m.get('listen_source')) == 'user_session' else ''}>用户会话</option></select></div></div>
<small class=field-hint>Bot 需要被拉进群；用户会话适合 Bot 拉不进去的群。选「用户会话」时，需在「Bot / 面板设置」填写 TG_API_ID、TG_API_HASH、TG_API_SESSION 并重启。</small>
...
<div class=grid><div><label>AI 超时（秒）</label><input name=ai_timeout_seconds type=number min=1 value='{html_escape(m.get('ai_timeout_seconds',30))}'></div><div><label>最小推送间隔（秒）</label><input name=ai_min_interval_seconds type=number min=0 value='{html_escape(m.get('ai_min_interval_seconds',DEFAULT_GROUP_AI_MIN_INTERVAL_SECONDS))}'></div><div><label>摘要去重窗口（秒）</label><input name=ai_dedupe_window_seconds type=number min=0 value='{html_escape(m.get('ai_dedupe_window_seconds',DEFAULT_GROUP_AI_DEDUPE_WINDOW_SECONDS))}'></div></div>
<label>AI 总结提示词（可选）</label><textarea name=ai_prompt placeholder='留空则使用默认总结提示词'>{html_escape(m.get('ai_prompt',''))}</textarea>
<label>关键词（一行一个，支持 /正则/）</label><textarea name=keywords placeholder='普通词写 VPS；正则写 /VPS|优惠|限免/'>{html_escape(keywords)}</textarea>
<small class=field-hint>普通词按大小写不敏感的包含匹配；/…/ 按正则匹配，例如 /VPS|优惠/、/^限免/。</small>
<label>排除词（一行一个，支持 /正则/）</label><textarea name=exclude_keywords placeholder='普通词写 求带；正则写 /(求带|带带)/'>{html_escape(exclude_keywords)}</textarea>
<small class=field-hint>命中排除词时直接跳过这条消息，规则写法与关键词一致。</small>
...
def create_panel_app() -> FastAPI:
    ...
    async def login_post(request: Request, username: str = Form(""), password: str = Form("")):
        ...
        if secrets.compare_digest(username, expected_user) and secrets.compare_digest(password, expected_pass):
            ...
            resp.set_cookie("tg_watchbot_session", session_token(expected_user), httponly=True, secure=panel_cookie_secure(request), samesite="lax", max_age=60 * 60 * 24 * 14)
            return resp
    ...
    async def index(_: str = Depends(panel_auth)) -> str:
        ...
        for i, m in enumerate(cfg.get("monitors") or []):
            ...
            rows.append(f"""<tr><td><span class=badge>{html_escape(m.get('type','web'))}</span></td><td><b>{html_escape(name)}</b><br><small>{html_escape(m.get('url',''))}</small></td><td>{html_escape(m.get('interval_seconds', DEFAULT_MONITOR_INTERVAL_SECONDS))}s<br><small>{tg}</small></td><td>{html_escape(', '.join(m.get('keywords') or []))}</td><td>{st_line}</td><td><a class=btn href='/monitor/{i}/edit'>编辑</a> <a class='btn ok' href='/monitor/{i}/preview'>预览</a> <a class='btn ok' href='/monitor/{i}/run'>检查</a> <a class='btn danger' href='/monitor/{i}/delete' onclick='return confirm("确定删除？")'>删除</a></td></tr>""")
    ...
    async def monitor_template(kind: str, _: str = Depends(panel_auth)) -> str:
        templates = {
            "nodeseek": {"name": "NodeSeek 新帖", "type": "rss", "url": "https://rss.nodeseek.com/", "interval_seconds": DEFAULT_MONITOR_INTERVAL_SECONDS, "keywords": ["NAT", "优惠", "补货", "VPS", "免费"], "forum": True, "notify_on": {"keyword_match": True, "new_item": True, "price_change": False, "stock_change": False}},
            "linuxdo": {"name": "Linux.do 最新", "type": "rss", "url": "https://linux.do/latest.rss", "interval_seconds": DEFAULT_MONITOR_INTERVAL_SECONDS, "keywords": ["Claude", "Codex", "API", "VPS", "NAT"], "forum": True, "notify_on": {"keyword_match": True, "new_item": True, "price_change": False, "stock_change": False}},
            "linuxdo-resource": {"name": "Linux.do 资源荟萃", "type": "rss", "url": "https://linux.do/c/resource/14.rss", "interval_seconds": DEFAULT_MONITOR_INTERVAL_SECONDS, "keywords": ["免费", "开源", "API", "Claude"], "forum": True, "notify_on": {"keyword_match": True, "new_item": True, "price_change": False, "stock_change": False}},
        }
    ...
    async def monitor_preview(idx: int, _: str = Depends(panel_auth)) -> str:
        ...
        try:
            ...
            for it in items[:15]:
                ...
                hits = item_keyword_hits(it, m.get("keywords") or [])
...
<div class=form-actions><button class='btn primary' type=submit>发送消息</button> <a class=btn href='/inbox'>查看收件箱</a></div></form></div>"""
    ...
    async def inbox_page(_: str = Depends(panel_auth)) -> str:
        ...
        for r in rows:
            ...
            elif (r["error"] or "").startswith("spam-hit:"):
                status_txt, status_cls = "广告命中", "warn"
            else:
...
<input type=hidden name=WEB_PANEL_ENABLED value='true'><div class=form-actions><button class='btn primary' type=submit>保存配置</button> <a class=btn href='/restart'>重启机器人</a></div></form></div>"""
    ...
    async def rules_page(_: str = Depends(panel_auth)) -> str:
        ...
        body = f"""<div class=card><h2>私聊广告拦截</h2><p class=muted>只拦截用户私聊 Bot 的双向对话消息，不影响 RSS/Web 监控关键词。监控内容过滤请使用监控配置里的屏蔽词。关键词支持普通词和 /正则/。命中且开启自动拉黑时：静默拉黑，不转发、不通知管理员。</p><form method=post>
...
<label>广告关键词（一行一个，支持 /正则/）</label><textarea name=keywords placeholder='普通词写 博彩；正则写 /(博彩|彩票|开户)/'>{html_escape(keywords)}</textarea>
<small class=field-hint>普通词按大小写不敏感的包含匹配；/…/ 按正则匹配，例如 /博彩|开户/、/^领红包/。</small>
